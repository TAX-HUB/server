...
const createNewUser = asyncHandler(async (req, res) => {
  ...
  if (duplicate) return res.status(409).json({ message: 'this email already in use...!' })
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  if (req.file && req.file.filename) req.body.profileImage = `users/${req.file.filename}`
  const user = await User.findById(req.params.id)
  if (!user) return res.status(400).json({ message: 'User not found' })
  const duplicate = await User.findOne({ _id: { $ne: req.params.id }, email: req.body.email })
  if (duplicate) return res.status(400).json({ success: false, error: `This email already in use ..!` })
  if (password) {
